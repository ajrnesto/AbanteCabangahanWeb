...
function getRequestsData() {
	...
	if (firstName && lastName) {
		qryRequest = query(refRequest, where("firstName", "==", firstName), where("lastName", "==", lastName), where("documentType", "==", "RESIDENCY"), orderBy("timestamp", "desc"));
	}
	else if (firstName && !lastName) {
		qryRequest = query(refRequest, where("firstName", "==", firstName), where("documentType", "==", "RESIDENCY"), orderBy("timestamp", "desc")) ;
	}
	else if (!firstName && lastName) {
		qryRequest = query(refRequest, where("lastName", "==", lastName), where("documentType", "==", "RESIDENCY"), orderBy("timestamp", "desc")) ;
	}
	else if (uid) {
		qryRequest = query(refRequest, where("userUid", "==", uid), where("documentType", "==", "RESIDENCY"), orderBy("timestamp", "desc")) ;
	}
	else if (!firstName && !lastName && !uid) {
		qryRequest = query(refRequest, where("documentType", "==", "RESIDENCY"), orderBy("timestamp", "desc")) ;
	}
...
function reqPrint(firstName, middleName, lastName, age, civilStatus, timestamp) {
	localStorage.setItem("service", "residency");
