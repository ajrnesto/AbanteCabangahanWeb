...
function getRequestsData() {
	const refId = ref(db, "indigencyRequests");
...
function renderTable(uid, firstName, middleName, lastName, age, civilStatus, timestamp, status) {
    ...
    rowId.appendChild(cellAge);
    rowId.appendChild(cellCivilStatus);
...
function btnAction(requestUid, requestStatus) {
    ...
    const refRequestStatus = ref(db, "indigencyRequests/"+requestUid+"/status");
    ...
    if (requestStatus < 2) {
        ...
        // if status == 2 or 3
        const refRequest = ref(db, "indigencyRequests/"+requestUid);
		remove(refRequest);
    }
}