...
function renderTable(uid, firstName, middleName, lastName, birthdate, purok, timestamp, status) {
    ...
    rowId.appendChild(cellBirthdate);
    rowId.appendChild(cellAddress);
...
function btnAction(requestUid, requestStatus) {
    ...
    const refRequestStatus = ref(db, "idRequests/"+requestUid+"/status");
    ...
    if (requestStatus < 2) {
        ...
        // if status == 2 or 3
        const refRequest = ref(db, "idRequests/"+requestUid);
		remove(refRequest);
    }
}