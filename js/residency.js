...
function getRequestsData() {
	const refId = ref(db, "residencyRequests");
...
function btnAction(requestUid, requestStatus) {
    ...
    const refRequestStatus = ref(db, "residencyRequests/"+requestUid+"/status");
    ...
    if (requestStatus < 2) {
        ...
        // if status == 2 or 3
        const refRequest = ref(db, "residencyRequests/"+requestUid);
		remove(refRequest);
    }
}